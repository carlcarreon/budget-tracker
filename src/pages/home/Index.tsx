...
export function HomePage() {
  ...
  useEffect(() => {
    ...
    const loadHome = async () => {
      ...
      if (expenseResult.error) {
        console.error("Failed to load expenses:", expenseResult.error);
      }

      if (goalResult.error) {
        console.error("Failed to load savings:", goalResult.error);
      }

      if (paylaterResult.error) {
        console.error("Failed to load paylater:", paylaterResult.error);
      }

      if (incomeResult.error) {
        console.error("Failed to load income:", incomeResult.error);
      }

      setOrders((orderResult.data ?? []).map(mapOrderRow));

      setExpenses((expenseResult.data ?? []).map(mapExpenseRow));

      setSavingGoals((goalResult.data ?? []).map(mapSavingGoalRow));

      setPaylaters((paylaterResult.data ?? []).map(mapPaylaterRow));

      setIncomes((incomeResult.data ?? []).map(mapIncomeRow));
    };
  ...
  const paylaterCount = paylaters.length;
  ...
  const todayIncome = useMemo(() => {
    ...
    return incomes.reduce(
      (sum, income) =>
        isSameDay(income.createdAt, today) ? sum + income.amount : sum,
      0,
    );
  }, [incomes]);
  ...
  const todayExpenses = useMemo(() => {
    ...
    return expenses.reduce(
      (sum, expense) =>
        isSameDay(expense.time, today) ? sum + expense.amount : sum,
      0,
    );
  }, [expenses]);
  ...
  const recentTransactions = useMemo(() => {
    ...
    return [...expenseTransactions, ...incomeTransactions]
      .filter((transaction) => Number.isFinite(transaction.date))
  ...
  const handleBalanceSubmit = async (payload: { amount: number }) => {
    if (!user) {
      return;
    }
    ...
    setIncomes((current) => [...current, mapIncomeRow(data)]);
  };
  ...
  const handleExpenseSubmit = async (payload: {
    ...
    setExpenses((current) => [...current, mapExpenseRow(data)]);
  };
  ...
  const handleOrderSubmit = async (payload: {
    ...
    setOrders((current) => [...current, mapOrderRow(data)]);
  };
  ...
  const handlePaylaterSubmit = async (payload: {
    ...
    if (error || !data) {
      console.error("Failed to add paylater:", error);
      return;
    }

    setPaylaters((current) => [...current, mapPaylaterRow(data)]);
  };
  ...
  return (
    <div className="space-y-2">
      ...
      <QuickActions
        ...
        onPaylaterSubmit={handlePaylaterSubmit}
      />
          ...
          <Card className="border-slate-200 bg-white py-2 shadow-sm">
            <CardContent className="space-y-2 px-2">
              <div className="flex items-center gap-2">
                ...
                <p className="text-md font-semibold text-slate-900">Orders</p>
              </div>
